"""Seed da tabela app.ref_etnias com a lista oficial DATASUS/e-SUS APS

Revision ID: 0013_seed_etnias
Revises: 0012_reference_tables
Create Date: 2026-04-16

Fonte: Dicionário de Dados LEDI do e-SUS APS (Laboratório Bridge/UFSC),
    referência oficial do Ministério da Saúde para integração do e-SUS APS.

Total: 406 etnias.
- Códigos 0001–0264: etnias originais
- Códigos X265–X405: extensões (sem X270 — gap oficial)
- Códigos 0304 (PURI) e 0315 (WARAO): adições recentes
"""
...
revision: str = "0013_seed_etnias"
down_revision: str | None = "0012_reference_tables"
...
def upgrade() -> None:
    values = ",\n".join(
        ...
        for cod, desc in ETNIAS
    )
    op.execute(f"""
        INSERT INTO app.ref_etnias (id, codigo, descricao, is_system, active) VALUES
        {values}
    """)


def downgrade() -> None:
    codes = ",".join(f"'{c}'" for c, _ in ETNIAS)
    op.execute(f"DELETE FROM app.ref_etnias WHERE codigo IN ({codes})")
