...
import { Search, X, ChevronLeft, ChevronRight, ChevronUp, ChevronDown, ChevronsUpDown, Shield } from 'lucide-react'
import { PageHeader } from '../../components/shared/PageHeader'
import { sigtapSearchApi, type HabilitacaoItem, type HabilitacaoProcedimentoItem } from '../../api/sigtap-search'
...
export function OpsHabilitacaoSearchPage() {
  const [search, setSearch] = useState('')
  const debouncedSearch = useDebounced(search, 300)
  const [page, setPage] = useState(1)
  const [items, setItems] = useState<HabilitacaoItem[]>([])
  const [total, setTotal] = useState(0)
  const [loading, setLoading] = useState(true)
  const [modalHab, setModalHab] = useState<HabilitacaoItem | null>(null)
  ...
  const reload = useCallback(async () => {
    setLoading(true)
    try {
      const res = await sigtapSearchApi.habilitacoes({ search: debouncedSearch || undefined, sort: sortField, dir: sortDir, page, pageSize: PAGE_SIZE })
      setItems(res.items)
      setTotal(res.total)
    } catch (e) {
  ...
  return (
    <div className="space-y-5">
      <PageHeader title="Habilitações × Procedimentos" subtitle={`${total.toLocaleString('pt-BR')} habilitações · selecione uma para ver os procedimentos`} back="/ops/pesquisas" />
      <div className="flex gap-2">
        <div className="relative flex-1">
          ...
          <input type="text" placeholder="Buscar por código ou descrição..." value={search} onChange={e => setSearch(e.target.value)}
            className="w-full pl-8 pr-8 py-2 text-sm bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg outline-none focus:border-indigo-400 transition-colors text-slate-800 dark:text-slate-200 placeholder-slate-400" />
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                {items.map(item => (
                  <tr key={item.codigo} className="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors">
                    <td className="px-4 py-3"><code className="text-xs font-mono font-bold text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-950/40 px-2 py-0.5 rounded">{item.codigo}</code></td>
                    <td className="px-4 py-3 text-slate-700 dark:text-slate-300">{item.descricao}</td>
                    <td className="px-4 py-3 text-center">
                      <span className="text-xs font-semibold text-slate-600 dark:text-slate-300 bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded-full tabular-nums">{item.totalProcedimentos.toLocaleString('pt-BR')}</span>
                    </td>
                    <td className="px-4 py-3 text-right">
                      <button onClick={() => setModalHab(item)} className="inline-flex items-center gap-1 text-xs text-indigo-600 hover:text-indigo-800 dark:text-indigo-400 dark:hover:text-indigo-200 font-medium">
                        <Shield size={12} /> Procedimentos
                      </button>
                    </td>
                  </tr>
                ))}
      ...
      {modalHab && <ProcedimentosModal hab={modalHab} onClose={() => setModalHab(null)} />}
    </div>
  )
}

function ProcedimentosModal({ hab, onClose }: { hab: HabilitacaoItem; onClose: () => void }) {
  const [search, setSearch] = useState('')
  const debouncedSearch = useDebounced(search, 300)
  const [page, setPage] = useState(1)
  const [items, setItems] = useState<HabilitacaoProcedimentoItem[]>([])
  const [total, setTotal] = useState(0)
  const [loading, setLoading] = useState(true)
  ...
  const reload = useCallback(async () => {
    setLoading(true)
    try {
      const res = await sigtapSearchApi.habilitacaoProcedimentos({ codigoHabilitacao: hab.codigo, search: debouncedSearch || undefined, page, pageSize: MODAL_PAGE_SIZE })
      setItems(res.items); setTotal(res.total)
    } catch (e) { toast.error('Falha ao carregar procedimentos', e instanceof HttpError ? e.message : ''); setItems([]); setTotal(0) }
    finally { setLoading(false) }
  }, [hab.codigo, debouncedSearch, page])
  useEffect(() => { void reload() }, [reload])
      ...
      <div className="relative bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-2xl w-full max-w-4xl max-h-[85vh] flex flex-col" onClick={e => e.stopPropagation()}>
        <div className="px-5 py-4 border-b border-slate-200 dark:border-slate-800 shrink-0">
          <div className="flex items-start justify-between gap-3">
            <div>
              <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-widest">Procedimentos da habilitação</p>
              <h2 className="text-base font-semibold mt-0.5"><code className="text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-950/40 px-1.5 py-0.5 rounded text-sm font-mono mr-2">{hab.codigo}</code>{hab.descricao}</h2>
          ...
          <div className="relative mt-3">
            ...
            <input type="text" placeholder="Filtrar procedimentos..." value={search} onChange={e => setSearch(e.target.value)}
              className="w-full pl-8 pr-8 py-2 text-sm bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg outline-none focus:border-indigo-400 transition-colors text-slate-800 dark:text-slate-200 placeholder-slate-400" />
            {search && <button onClick={() => setSearch('')} className="absolute right-2.5 top-1/2 -translate-y-1/2 text-slate-300 hover:text-slate-500"><X size={13} /></button>}
          </div>
        </div>
            ...
            <table className="w-full text-sm">
              <thead className="sticky top-0 bg-slate-50 dark:bg-slate-800/80 backdrop-blur-sm">
                <tr className="border-b border-slate-100 dark:border-slate-800">
                  <Th>Código</Th><Th>Procedimento</Th><Th>Compl.</Th><Th>Grupo Hab.</Th><Th className="text-right">Valor SH</Th><Th className="text-right">Total</Th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                {items.map((p, i) => (
                  <tr key={`${p.codigoProcedimento}-${i}`} className="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors">
                    ...
                    <td className="px-4 py-2.5"><code className="text-xs font-mono text-slate-500 bg-slate-100 dark:bg-slate-800 px-1.5 py-0.5 rounded">{p.codigoGrupoHabilitacao}</code></td>
  ...
  return (
    <th className={cn('px-4 py-2.5 text-left', className)}>
      <button onClick={() => onSort(field)} className="inline-flex items-center gap-1 text-[11px] font-semibold text-slate-400 uppercase tracking-wider hover:text-slate-600 dark:hover:text-slate-200 transition-colors">
        {children}
        {active ? dir === 'asc' ? <ChevronUp size={11} className="text-indigo-500" /> : <ChevronDown size={11} className="text-indigo-500" /> : <ChevronsUpDown size={11} className="opacity-40" />}
      </button>
    </th>
  )
}
...
function Spinner() { return <svg className="animate-spin w-5 h-5 text-indigo-500" viewBox="0 0 24 24" fill="none"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8z" /></svg> }
