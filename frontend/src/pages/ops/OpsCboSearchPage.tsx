...
import { Search, X, ChevronLeft, ChevronRight, ChevronUp, ChevronDown, ChevronsUpDown, Briefcase } from 'lucide-react'
import { PageHeader } from '../../components/shared/PageHeader'
import { sigtapSearchApi, type CboItem, type CboProcedimentoItem } from '../../api/sigtap-search'
...
export function OpsCboSearchPage() {
  ...
  const [modalCbo, setModalCbo] = useState<CboItem | null>(null)
  ...
  const reload = useCallback(async () => {
    setLoading(true)
    try {
      const res = await sigtapSearchApi.cbos({ search: debouncedSearch || undefined, sort: sortField, dir: sortDir, page, pageSize: PAGE_SIZE })
      setItems(res.items)
      setTotal(res.total)
    } catch (e) {
  ...
  return (
    <div className="space-y-5">
      <PageHeader title="CBO × Procedimentos" subtitle={`${total.toLocaleString('pt-BR')} ocupações · selecione uma para ver os procedimentos`} back="/ops/pesquisas" />
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                {items.map(item => (
                  <tr key={item.codigo} className="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors">
                    <td className="px-4 py-3"><code className="text-xs font-mono font-bold text-sky-600 dark:text-sky-400 bg-sky-50 dark:bg-sky-950/40 px-2 py-0.5 rounded">{item.codigo}</code></td>
                    ...
                    <td className="px-4 py-3 text-right">
                      <button onClick={() => setModalCbo(item)} className="inline-flex items-center gap-1 text-xs text-sky-600 hover:text-sky-800 dark:text-sky-400 dark:hover:text-sky-200 font-medium">
                        <Briefcase size={12} /> Procedimentos
                      </button>
                    </td>
                  </tr>
                ))}
      ...
      {modalCbo && <ProcedimentosModal cbo={modalCbo} onClose={() => setModalCbo(null)} />}
    </div>
  )
}

function ProcedimentosModal({ cbo, onClose }: { cbo: CboItem; onClose: () => void }) {
  const [search, setSearch] = useState('')
  const debouncedSearch = useDebounced(search, 300)
  const [page, setPage] = useState(1)
  const [items, setItems] = useState<CboProcedimentoItem[]>([])
  const [total, setTotal] = useState(0)
  const [loading, setLoading] = useState(true)
  ...
  const reload = useCallback(async () => {
    setLoading(true)
    try {
      const res = await sigtapSearchApi.cboProcedimentos({ codigoCbo: cbo.codigo, search: debouncedSearch || undefined, page, pageSize: MODAL_PAGE_SIZE })
      setItems(res.items); setTotal(res.total)
    } catch (e) { toast.error('Falha ao carregar procedimentos', e instanceof HttpError ? e.message : ''); setItems([]); setTotal(0) }
    finally { setLoading(false) }
  }, [cbo.codigo, debouncedSearch, page])
  useEffect(() => { void reload() }, [reload])
      ...
      <div className="relative bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-2xl w-full max-w-4xl max-h-[85vh] flex flex-col" onClick={e => e.stopPropagation()}>
        <div className="px-5 py-4 border-b border-slate-200 dark:border-slate-800 shrink-0">
          <div className="flex items-start justify-between gap-3">
            <div>
              <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-widest">Procedimentos habilitados</p>
              <h2 className="text-base font-semibold mt-0.5"><code className="text-sky-600 dark:text-sky-400 bg-sky-50 dark:bg-sky-950/40 px-1.5 py-0.5 rounded text-sm font-mono mr-2">{cbo.codigo}</code>{cbo.descricao}</h2>
            ...
            <table className="w-full text-sm">
              <thead className="sticky top-0 bg-slate-50 dark:bg-slate-800/80 backdrop-blur-sm">
                <tr className="border-b border-slate-100 dark:border-slate-800">
                  <Th>Código</Th><Th>Procedimento</Th><Th>Compl.</Th><Th className="text-right">Valor SH</Th><Th className="text-right">Valor SA</Th><Th className="text-right">Total</Th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                {items.map(p => (
                  <tr key={p.codigoProcedimento} className="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors">
                    ...
                    <td className="px-4 py-2.5 text-right text-xs text-slate-500 tabular-nums">{formatBRL(p.valorSa)}</td>
  ...
  return (
    <th className={cn('px-4 py-2.5 text-left', className)}>
      <button onClick={() => onSort(field)} className="inline-flex items-center gap-1 text-[11px] font-semibold text-slate-400 uppercase tracking-wider hover:text-slate-600 dark:hover:text-slate-200 transition-colors">
        {children}
        {active ? dir === 'asc' ? <ChevronUp size={11} className="text-sky-500" /> : <ChevronDown size={11} className="text-sky-500" /> : <ChevronsUpDown size={11} className="opacity-40" />}
      </button>
    </th>
  )
}
