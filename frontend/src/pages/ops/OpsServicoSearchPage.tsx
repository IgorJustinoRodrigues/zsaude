...
export function OpsServicoSearchPage() {
  ...
  const toggleSort = (field: string) => {
    ...
    else { setSortField(field); setSortDir(field === 'totalClassificacoes' ? 'desc' : 'asc') }
  }
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50">
                  ...
                  <SortTh field="totalClassificacoes" current={sortField} dir={sortDir} onSort={toggleSort} className="text-center">Classificações</SortTh>
                ...
                {items.map(item => (
                  <tr key={item.codigo} className="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors">
                    ...
                    <td className="px-4 py-3 text-right">
                      <button onClick={() => setModalServico(item)} className="inline-flex items-center gap-1 text-xs text-teal-600 hover:text-teal-800 dark:text-teal-400 dark:hover:text-teal-200 font-medium">
