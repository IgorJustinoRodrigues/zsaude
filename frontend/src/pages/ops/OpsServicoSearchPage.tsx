...
import { Search, X, ChevronLeft, ChevronRight, ChevronUp, ChevronDown, ChevronsUpDown, Layers } from 'lucide-react'
import { PageHeader } from '../../components/shared/PageHeader'
import { sigtapSearchApi, type ServicoItem, type ServicoProcedimentoItem } from '../../api/sigtap-search'
...
const complexCls = (c: string) => ({ '1': 'bg-emerald-50 text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-400', '2': 'bg-amber-50 text-amber-700 dark:bg-amber-950/40 dark:text-amber-400', '3': 'bg-red-50 text-red-700 dark:bg-red-950/40 dark:text-red-400' }[c] ?? 'bg-slate-100 text-slate-500')
type SortDir = 'asc' | 'desc'

export function OpsServicoSearchPage() {
  ...
  const [sortField, setSortField] = useState('totalProcedimentos')
  ...
  const reload = useCallback(async () => {
    setLoading(true)
    try {
      const res = await sigtapSearchApi.servicos({ search: debouncedSearch || undefined, sort: sortField, dir: sortDir, page, pageSize: PAGE_SIZE })
      setItems(res.items)
      setTotal(res.total)
    } catch (e) {
  ...
  const toggleSort = (field: string) => {
    ...
    else { setSortField(field); setSortDir(field === 'totalClassificacoes' || field === 'totalProcedimentos' ? 'desc' : 'asc') }
  }
  ...
  return (
    <div className="space-y-5">
      <PageHeader title="Serviços × Procedimentos" subtitle={`${total.toLocaleString('pt-BR')} serviços · selecione um para ver os procedimentos`} back="/ops/pesquisas" />
      <div className="flex gap-2">
        <div className="relative flex-1">
          ...
          <input type="text" placeholder="Buscar por código ou descrição..." value={search} onChange={e => setSearch(e.target.value)}
            className="w-full pl-8 pr-8 py-2 text-sm bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg outline-none focus:border-teal-400 transition-colors text-slate-800 dark:text-slate-200 placeholder-slate-400" />
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50">
                  ...
                  <SortTh field="totalClassificacoes" current={sortField} dir={sortDir} onSort={toggleSort} className="text-center">Classif.</SortTh>
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                {items.map(item => (
                  <tr key={item.codigo} className="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors">
                    <td className="px-4 py-3"><code className="text-xs font-mono font-bold text-teal-600 dark:text-teal-400 bg-teal-50 dark:bg-teal-950/40 px-2 py-0.5 rounded">{item.codigo}</code></td>
                    <td className="px-4 py-3 text-slate-700 dark:text-slate-300">{item.descricao}</td>
                    <td className="px-4 py-3 text-center">
                      <span className="text-xs font-semibold text-slate-600 dark:text-slate-300 bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded-full tabular-nums">{item.totalClassificacoes.toLocaleString('pt-BR')}</span>
                    </td>
                    <td className="px-4 py-3 text-center">
                      <span className={cn('text-xs font-semibold px-2 py-0.5 rounded-full tabular-nums',
                        item.totalProcedimentos > 0
                          ? 'text-teal-700 dark:text-teal-300 bg-teal-50 dark:bg-teal-950/40'
                          : 'text-slate-400 bg-slate-100 dark:bg-slate-800'
                      )}>{item.totalProcedimentos.toLocaleString('pt-BR')}</span>
                    </td>
                    <td className="px-4 py-3 text-right">
                      <button onClick={() => setModalServico(item)} disabled={item.totalProcedimentos === 0}
                        className="inline-flex items-center gap-1 text-xs text-teal-600 hover:text-teal-800 dark:text-teal-400 dark:hover:text-teal-200 font-medium disabled:opacity-30 disabled:cursor-not-allowed disabled:hover:text-teal-600">
                        <Layers size={12} /> Procedimentos
                      </button>
                    </td>
                  </tr>
                ))}
      ...
      {modalServico && <ProcedimentosModal servico={modalServico} onClose={() => setModalServico(null)} />}
    </div>
  )
}

function ProcedimentosModal({ servico, onClose }: { servico: ServicoItem; onClose: () => void }) {
  const [search, setSearch] = useState('')
  const debouncedSearch = useDebounced(search, 300)
  const [page, setPage] = useState(1)
  const [items, setItems] = useState<ServicoProcedimentoItem[]>([])
  const [total, setTotal] = useState(0)
  const [loading, setLoading] = useState(true)
  ...
  const reload = useCallback(async () => {
    setLoading(true)
    try {
      const res = await sigtapSearchApi.servicoProcedimentos({ codigoServico: servico.codigo, search: debouncedSearch || undefined, page, pageSize: MODAL_PAGE_SIZE })
      setItems(res.items); setTotal(res.total)
    } catch (e) { toast.error('Falha ao carregar procedimentos', e instanceof HttpError ? e.message : ''); setItems([]); setTotal(0) }
    finally { setLoading(false) }
  }, [servico.codigo, debouncedSearch, page])
  useEffect(() => { void reload() }, [reload])
      ...
      <div className="relative bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-2xl w-full max-w-4xl max-h-[85vh] flex flex-col" onClick={e => e.stopPropagation()}>
        <div className="px-5 py-4 border-b border-slate-200 dark:border-slate-800 shrink-0">
          <div className="flex items-start justify-between gap-3">
            <div>
              <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-widest">Procedimentos do serviço</p>
              <h2 className="text-base font-semibold mt-0.5"><code className="text-teal-600 dark:text-teal-400 bg-teal-50 dark:bg-teal-950/40 px-1.5 py-0.5 rounded text-sm font-mono mr-2">{servico.codigo}</code>{servico.descricao}</h2>
          ...
          <div className="relative mt-3">
            ...
            <input type="text" placeholder="Filtrar procedimentos..." value={search} onChange={e => setSearch(e.target.value)}
              className="w-full pl-8 pr-8 py-2 text-sm bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg outline-none focus:border-teal-400 transition-colors text-slate-800 dark:text-slate-200 placeholder-slate-400" />
            {search && <button onClick={() => setSearch('')} className="absolute right-2.5 top-1/2 -translate-y-1/2 text-slate-300 hover:text-slate-500"><X size={13} /></button>}
          </div>
        </div>
        <div className="flex-1 overflow-y-auto">
          {loading ? <div className="flex items-center justify-center py-12"><Spinner /></div>
          : items.length === 0 ? <div className="flex flex-col items-center justify-center py-12 text-slate-400"><Search size={24} className="mb-2 opacity-30" /><p className="text-sm">Nenhum procedimento encontrado</p></div>
          : (
            <table className="w-full text-sm">
              <thead className="sticky top-0 bg-slate-50 dark:bg-slate-800/80 backdrop-blur-sm">
                <tr className="border-b border-slate-100 dark:border-slate-800">
                  <Th>Código</Th><Th>Procedimento</Th><Th>Classificação</Th><Th>Compl.</Th><Th className="text-right">Valor SH</Th><Th className="text-right">Total</Th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                {items.map((p, i) => (
                  <tr key={`${p.codigoProcedimento}-${p.codigoClassificacao}-${i}`} className="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors">
                    <td className="px-4 py-2.5"><code className="text-xs font-mono font-bold text-violet-600 dark:text-violet-400 bg-violet-50 dark:bg-violet-950/40 px-1.5 py-0.5 rounded">{p.codigoProcedimento}</code></td>
                    <td className="px-4 py-2.5 text-slate-700 dark:text-slate-300 max-w-xs truncate">{p.nomeProcedimento}</td>
                    <td className="px-4 py-2.5"><code className="text-xs font-mono text-slate-500 bg-slate-100 dark:bg-slate-800 px-1.5 py-0.5 rounded">{p.codigoClassificacao}</code></td>
  ...
  return (
    <th className={cn('px-4 py-2.5 text-left', className)}>
      <button onClick={() => onSort(field)} className="inline-flex items-center gap-1 text-[11px] font-semibold text-slate-400 uppercase tracking-wider hover:text-slate-600 dark:hover:text-slate-200 transition-colors">
        {children}
        {active ? dir === 'asc' ? <ChevronUp size={11} className="text-teal-500" /> : <ChevronDown size={11} className="text-teal-500" /> : <ChevronsUpDown size={11} className="opacity-40" />}
      </button>
    </th>
  )
}
...
function Spinner() { return <svg className="animate-spin w-5 h-5 text-teal-500" viewBox="0 0 24 24" fill="none"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8z" /></svg> }
