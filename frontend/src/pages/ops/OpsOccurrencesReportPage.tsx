...
import {
  ...
  LogIn, Trash2, FileEdit, ShieldAlert, KeyRound, ShieldOff, Download as DownloadIcon,
...
const OCCURRENCE_SEVERITIES: LogSeverity[] = ['warning', 'error', 'critical']

const SEV_META: Record<LogSeverity, {
  label: string; icon: React.ReactNode
  badge: string; row: string; dot: string
}> = {
  info: {
    label: 'Info', icon: null,
    badge: '', row: '', dot: '',
  },
  warning: {
    label: 'Aviso',
  ...
  error: {
    label: 'Erro',
    icon: <TriangleAlert size={13} />,
    badge: 'bg-red-50 text-red-700 dark:bg-red-950/40 dark:text-red-400 border-red-200 dark:border-red-800',
  ...
  critical: {
    label: 'Crítico',
    icon: <Flame size={13} />,
    badge: 'bg-red-100 text-red-800 dark:bg-red-950/60 dark:text-red-300 border-red-300 dark:border-red-700',
...
const ACTION_META: Record<LogAction, { label: string; icon: React.ReactNode }> = {
  ...
  export:            { label: 'Exportação',     icon: <DownloadIcon size={12} /> },
  print:             { label: 'Impressão',      icon: null },
...
const ACTION_COLOR: Record<LogAction, string> = {
  ...
  print:             'text-slate-600 bg-slate-100 dark:bg-slate-800',
...
type SortField = 'at' | 'userName' | 'module' | 'severity' | 'action'
...
export function OpsOccurrencesReportPage() {
  ...
  const occUsers = useMemo(() => {
    const ids = [...new Set(baseOccurrences.map(l => l.userId))]
    return ids.map(id => ({
      id, name: baseOccurrences.find(l => l.userId === id)?.userName ?? id,
    })).sort((a, b) => a.name.localeCompare(b.name))
  }, [baseOccurrences])

  // Ações presentes nas ocorrências
  const occActions = useMemo(() => {
    const acts = [...new Set(baseOccurrences.map(l => l.action))] as LogAction[]
    return acts.sort()
  }, [baseOccurrences])
  ...
  const filtered = useMemo(() => {
    ...
    const now = Date.now()
    const list = baseOccurrences.filter(l => {
  ...
  }, [baseOccurrences, search, filterSeverity, filterModule, filterAction, filterUser, filterPeriod, dateFrom, dateTo, sortField, sortDir])
  ...
  // Top padrões por ação+módulo
  const topPatterns = useMemo(() => {
    const map = new Map<string, { key: string; action: LogAction; module: string; count: number; lastAt: Date }>()
    filtered.forEach(l => {
      const key = `${l.action}::${l.module}`
      if (!map.has(key)) map.set(key, { key, action: l.action, module: l.module, count: 0, lastAt: l.at })
      const e = map.get(key)!
      e.count++
  ...
  const hasFilter = filterSeverity.length > 0 || filterModule.length > 0 ||
    filterAction.length > 0 || filterUser.length > 0 ||
    filterPeriod !== 0 || !!dateFrom || !!dateTo

  const filterCount = [
    filterSeverity.length > 0, filterModule.length > 0,
    filterAction.length > 0,   filterUser.length > 0,
    filterPeriod !== 0 || !!dateFrom || !!dateTo,
  ].filter(Boolean).length

  const clearFilters = () => {
    setFilterSeverity([]); setFilterModule([])
  ...
  function exportCsv() {
    const rows = [
      ['ID', 'Hash', 'Data', 'Hora', 'Severidade', 'Usuário', 'Ação', 'Módulo', 'Recurso', 'Recurso ID', 'Descrição', 'IP'],
      ...filtered.map(l => [
        l.id, l.hash,
        fmtDate(l.at), fmtTime(l.at),
        SEV_META[l.severity].label,
        l.userName,
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
        <div>
          ...
          <h1 className="text-lg md:text-xl font-bold text-slate-900 dark:text-white flex items-center gap-2">
            <OccIcon size={20} className="text-red-500" />
            Relatório de Ocorrências
          </h1>
          <p className="text-sm text-slate-500 mt-0.5">Avisos, erros e eventos críticos registrados no sistema</p>
        </div>
        <button
          onClick={exportCsv}
          ...
          className={cn(
            ...
            filtered.length > 0
              ? 'border-red-300 text-red-600 bg-red-50 dark:bg-red-950/30 dark:text-red-400 dark:border-red-700 hover:bg-red-100 dark:hover:bg-red-950/50'
        ...
        >
          ...
          Exportar CSV
          {filtered.length > 0 && (
            <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded-full bg-red-100 dark:bg-red-900/50 text-red-600 dark:text-red-400">
              {filtered.length}
            </span>
          )}
        </button>
      ...
      {topPatterns.length > 0 && (
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-4">
          <div className="flex items-center gap-2 mb-4">
            ...
            <p className="text-sm font-semibold text-slate-700 dark:text-slate-200">Padrões mais frequentes</p>
            <span className="text-[10px] text-slate-400 ml-1">no período filtrado</span>
          </div>
          <div className="space-y-2.5">
            {topPatterns.map((p, i) => {
              const modColor = MODULE_COLOR[p.module] ?? '#6b7280'
                ...
                <div key={p.key}>
                  <div className="flex items-center justify-between gap-3 mb-1">
                    <div className="flex items-center gap-2 min-w-0">
                      ...
                      <div className={cn('inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-xs font-medium shrink-0', ACTION_COLOR[p.action])}>
                        {ACTION_META[p.action].icon}
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      <span className="text-xs font-bold text-slate-600 dark:text-slate-300">{p.count}×</span>
                    </div>
                  </div>
                  <div className="h-1.5 bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden ml-4">
                    <div
      ...
      <div className="flex flex-col sm:flex-row gap-2">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-8 pr-8 py-2 text-sm bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl outline-none focus:border-red-400 transition-colors text-slate-800 dark:text-slate-200 placeholder-slate-400"
          />
          {search && (
        ...
        <button
          ...
          className={cn(
            ...
            showFilter || hasFilter
              ? 'border-red-400 text-red-600 bg-red-50 dark:bg-red-950/30 dark:text-red-400 dark:border-red-700'
        ...
        >
          ...
          Filtros
          {hasFilter && (
            <span className="w-4 h-4 text-[9px] font-bold rounded-full bg-red-500 text-white flex items-center justify-center">
              {filterCount}
            </span>
          )}
        </button>
            ...
            <FilterSection label="Intervalo personalizado">
              <div className="flex items-center gap-2 flex-wrap">
                <input type="date" value={dateFrom}
                  ...
                  className="px-2.5 py-1.5 text-xs bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-700 rounded-lg outline-none focus:border-red-400 text-slate-700 dark:text-slate-200"
                />
              ...
              {OCCURRENCE_SEVERITIES.map(s => (
                <button
                  ...
                  className={cn(
                    ...
                    filterSeverity.includes(s) ? SEV_META[s].badge : 'bg-slate-100 dark:bg-slate-800 text-slate-500 border-transparent hover:bg-slate-200 dark:hover:bg-slate-700',
                  )}
                >
                  {SEV_META[s].icon}
                  {SEV_META[s].label}
      ...
      {filtered.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl gap-3 text-center">
          ...
          <p className="text-sm font-medium text-slate-600 dark:text-slate-300">Nenhuma ocorrência encontrada</p>
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden">
          ...
          <div className="hidden lg:block overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                {paginated.map(log => (
                  <OccRow key={log.id} log={log} onView={() => setSelected(log)} />
                ))}
              </tbody>
            </table>
          </div>
          ...
          <div className="lg:hidden divide-y divide-slate-100 dark:divide-slate-800">
            {paginated.map(log => (
              <OccMobileCard key={log.id} log={log} onView={() => setSelected(log)} />
            ))}
          </div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 px-5 py-3 border-t border-slate-100 dark:border-slate-800 bg-slate-50/60 dark:bg-slate-800/20">
            <div className="flex items-center gap-3 text-xs text-slate-500">
              <span>
                {((page - 1) * pageSize) + 1}–{Math.min(page * pageSize, filtered.length)} de {filtered.length} ocorrência{filtered.length !== 1 ? 's' : ''}
              </span>
...
function OccRow({ log, onView }: { log: SystemLog; onView: () => void }) {
  const sev      = SEV_META[log.severity]
  const act      = ACTION_META[log.action]
  const modColor = MODULE_COLOR[log.module] ?? '#6b7280'
  return (
    <tr className={cn('hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors', sev.row)}>
      <td className="px-4 py-3">
        <span className={cn('inline-flex items-center gap-1 text-xs font-medium px-2 py-0.5 rounded-full border', sev.badge)}>
          {sev.icon}
          {sev.label}
        </span>
      </td>
...
function OccMobileCard({ log, onView }: { log: SystemLog; onView: () => void }) {
  const sev      = SEV_META[log.severity]
  const act      = ACTION_META[log.action]
  const modColor = MODULE_COLOR[log.module] ?? '#6b7280'
  return (
    <div className={cn('px-4 py-3.5 hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors', sev.row)}>
      <div className="flex items-start gap-2.5">
        <span className={cn('inline-flex items-center gap-1 text-[11px] font-semibold px-2 py-0.5 rounded-full border shrink-0 mt-0.5', sev.badge)}>
          {sev.icon}
          {sev.label}
        </span>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={onClose}>
      <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 dark:border-slate-800">
          <div className="flex items-center gap-3">
            <div className={cn('w-8 h-8 rounded-lg flex items-center justify-center shrink-0', ACTION_COLOR[log.action])}>
              {act.icon ?? <TriangleAlert size={14} />}
            </div>
            <div>
        ...
        <div className="p-5 space-y-5 overflow-y-auto max-h-[70vh]">
          <div className="flex items-center gap-2 flex-wrap">
            <span className={cn('inline-flex items-center gap-1 text-xs font-medium px-2.5 py-1 rounded-full border', sev.badge)}>
              {sev.icon} {sev.label}
            </span>
          ...
          <ModalSection title="Detalhes da ocorrência">
    ...
    <th className="px-4 py-2.5 text-left">
      {sortable && field && onSort ? (
        <button onClick={() => onSort(field)}
          ...
          {children}
          {active
            ? dir === 'asc' ? <ChevronUp size={11} className="text-red-500" /> : <ChevronDown size={11} className="text-red-500" />
