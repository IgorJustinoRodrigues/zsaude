...
import { sigtapSearchApi, type CboProcedimentoItem } from '../../api/sigtap-search'
...
export function OpsCboProcedureSearchPage() {
  const [params] = useSearchParams()
  const cbo = params.get('cbo') ?? ''
  const cboDesc = params.get('desc') ?? ''
  ...
  const reload = useCallback(async () => {
    ...
    setLoading(true)
    try {
      const res = await sigtapSearchApi.cboProcedimentos({
        codigoCbo: cbo,
  ...
  return (
    <div className="space-y-5">
      <PageHeader
        title={`CBO ${cbo} — Procedimentos`}
        subtitle={cboDesc ? `${cboDesc} · ${total.toLocaleString('pt-BR')} procedimentos habilitados` : `${total.toLocaleString('pt-BR')} procedimentos habilitados`}
        back="/ops/pesquisas/cbo"
      />
      ...
      <div className="bg-violet-50 dark:bg-violet-950/30 border border-violet-200 dark:border-violet-900/60 rounded-xl p-4">
        <p className="text-sm text-violet-800 dark:text-violet-200">
          Mostrando procedimentos que a ocupação <strong>{cbo}</strong> está habilitada a executar conforme o SIGTAP vigente.
        </p>
      </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50">
                  ...
                  <Th className="text-right">Valor SP</Th>
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                {items.map(p => (
                  <tr key={p.codigoProcedimento} className="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors">
                    <td className="px-4 py-3"><code className="text-xs font-mono font-bold text-violet-600 dark:text-violet-400 bg-violet-50 dark:bg-violet-950/40 px-2 py-0.5 rounded">{p.codigoProcedimento}</code></td>
                    ...
                    <td className="px-4 py-3 text-right text-xs text-slate-500 tabular-nums">{formatBRL(p.valorSp)}</td>
...
function EmptyBox() {
  return <div className="flex flex-col items-center justify-center py-16 text-slate-400 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl"><Search size={28} className="mb-2 opacity-30" /><p className="text-sm">Nenhum procedimento encontrado para este CBO</p></div>
}
