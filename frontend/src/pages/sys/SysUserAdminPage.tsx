...
type Tab = 'users' | 'municipal' | 'birthdays'
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            Usuários MASTER e ADMIN da plataforma e, na aba "Dos municípios", os operacionais vinculados às unidades.
          </p>
        </div>
        {(tab === 'users' || tab === 'municipal') && (
          <button
      ...
      <div className="border-b border-slate-200 dark:border-slate-800 flex gap-1">
        ...
        <TabBtn active={tab === 'municipal'} onClick={() => setTab('municipal')} icon={<Building2 size={14} />}>
          Dos municípios
        </TabBtn>
      ...
      {tab === 'users' && <UsersTab navigate={navigate} currentUserId={currentUserId} />}
      {tab === 'municipal' && <MunicipalUsersTab navigate={navigate} />}
      {tab === 'birthdays' && <BirthdaysPanel viewBasePath="/sys/usuarios" accent="violet" />}
    </div>
  )
}
...
// ─── Tab "Dos municípios" — lista USERs (nível operacional) ─────────────────

function MunicipalUsersTab({
  navigate,
}: {
  navigate: ReturnType<typeof useNavigate>
}) {
  const [items, setItems] = useState<UserListItem[]>([])
  const [munOptions, setMunOptions] = useState<MunicipalityDto[]>([])
  ...
  useEffect(() => {
    directoryApi.listMunicipalities('all')
      .then(setMunOptions)
      .catch(e => toast.error('Falha ao carregar municípios',
  ...
  const load = useCallback(async () => {
    ...
    try {
      const r = await userApi.list({
        ...
        municipalityId: munId || undefined,
        pageSize: 100,
      })
      // Só USER — ADMIN/MASTER vão na outra aba.
      setItems(r.items.filter(u => u.level === 'user'))
    } catch (e) {
      const msg = e instanceof HttpError ? e.message : 'Erro ao carregar.'
      setError(msg)
      toast.error('Falha ao carregar', msg)
  ...
  const selectedMun = munOptions.find(m => m.id === munId)
    ...
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row gap-2">
        ...
        <div className="sm:w-72">
          <ComboBox
            ...
            options={munOptions.map<ComboBoxOption>(m => ({
              value: m.id, label: m.name, hint: m.state,
            }))}
          />
        </div>
        <ExportMenuButton<UserListItem>
          options={buildMunicipalExportOptions(items, selectedMun, branding)}
          pdfOrientation="landscape"
        />
      </div>
      ...
      {loading ? (
        ...
        <EmptyState icon={<Users size={32} />}
          label={munId
            ? 'Nenhum usuário operacional neste município.'
            : 'Nenhum usuário operacional encontrado.'} />
      ) : (
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden divide-y divide-slate-100 dark:divide-slate-800">
          {items.map(u => (
            <button key={u.id}
              ...
              <div className="flex items-center gap-2 shrink-0">
                <span className="inline-flex items-center gap-1 text-[10px] font-medium px-2 py-0.5 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400">
                  <Building2 size={10} />
                  {u.municipalityCount} {u.municipalityCount === 1 ? 'município' : 'municípios'}
                </span>
                <span className={cn('text-xs font-medium px-2 py-0.5 rounded-full', STATUS_STYLE[u.status] ?? '')}>
                  {u.status === 'Ativo' ? <UserCheck size={11} className="inline mr-1" /> : u.status === 'Bloqueado' ? <UserX size={11} className="inline mr-1" /> : null}
                  {u.status}
...
function buildMunicipalExportOptions(
  items: UserListItem[],
  mun: MunicipalityDto | undefined,
  branding?: ExportBranding,
): ExportOptions<UserListItem> {
  const scope = mun ? `${mun.name}/${mun.state}` : 'Todos os municípios'
  return {
    title: 'Usuários operacionais dos municípios',
    subtitle: `${scope} · ${items.length} ${items.length === 1 ? 'usuário' : 'usuários'}`,
    filename: `usuarios-municipios-${mun?.name.toLowerCase().replace(/\s+/g, '-') || 'todos'}`,
    rows: items,
    columns: [
      { header: 'Nome',     get: u => u.name },
      { header: 'E-mail',   get: u => u.email ?? '—' },
      { header: 'CPF',      get: u => u.cpf ?? '—', width: 110 },
      { header: 'CNS',      get: u => u.cns ?? '—', width: 130 },
      { header: 'Telefone', get: u => u.phone || '—', width: 110 },
      { header: 'Perfil',   get: u => u.primaryRole || '—' },
      { header: 'Municípios', get: u => String(u.municipalityCount), align: 'center', width: 70 },
      { header: 'Unidades',   get: u => String(u.facilityCount), align: 'center', width: 70 },
